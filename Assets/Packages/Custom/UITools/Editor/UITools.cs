...
public class UITools : EditorWindow {
	...
	private static void Init() {
		UITools window = GetWindow<UITools>();
		window.minSize = new Vector2(40F, 40F);
		window.Show();
	}

	private const string ROOT_PATH = "Assets/Editor/UITools/Res/";
	protected static readonly GUILayoutOption BTN_HEIGHT_OPTION = GUILayout.Height(30F);
	protected static readonly GUILayoutOption SEPARATOR_HEIGHT_OPTION = GUILayout.Height(8F);
	...
	private static void DrawSeparator() {
		GUILayout.Label("————————————————————", "CenteredLabel", SEPARATOR_HEIGHT_OPTION);
	}
	...
	private static void GroupUnpack() {
		...
		if (count > 0) {
			...
			Selection.objects = new List<Transform>(childrenSet)
					.FindAll(child => child)
					.ConvertAll(child => (UObject) child.gameObject)
					.ToArray();
		}
	}
	...
	private static (RectTransform, List<RectTransform>) GetBasedAndList() {
		List<RectTransform> list = GetSelectedList();
		RectTransform based = list.Count > 1 ? Selection.activeTransform as RectTransform : list[0].parent as RectTransform;
		if (based) {
			// 有父子关系的，父节点排前面。设置父节点会影响子节点，所以先设置父节点，再设置子节点。
			list.Sort((tans1, tans2) => tans1.IsChildOf(tans2) ? 1 : tans2.IsChildOf(tans1) ? -1 : 0);
			return (based, list);

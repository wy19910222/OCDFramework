 ...
 * @LastEditor: wangyun
 * @EditTime: 2022-04-18 21:23:39 596
 */
...
namespace Control {
	public abstract class BaseProgressCtrlVector2 : BaseProgressCtrl {
		[SerializeField, ReadOnly]
		private Vector2 m_FromValue;
		[SerializeField, ReadOnly]
		private Vector2 m_ToValue;
		...
		private void Reset() {
			...
			m_Curves = new []{m_CurveX, m_CurveY};
		}

		private Vector2 GetValue(float progress) {
		...
		private void SetValue(float progress, Vector2 value) {
			for (int part = 0; part < 2; part++) {
		...
		protected virtual Vector2 TargetValue { get; set; }
		...
		public override void Capture(float progress) {
			Vector2 targetValue = TargetValue;
			...
			(m_CurveX, m_CurveY) = (m_Curves[0], m_Curves[1]);
#endif
		}

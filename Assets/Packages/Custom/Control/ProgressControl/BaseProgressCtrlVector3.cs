...
namespace Control {
	public abstract class BaseProgressCtrlVector3 : BaseProgressCtrl {
		[SerializeField, ReadOnly]
		private Vector3 m_FromValue;
		[SerializeField, ReadOnly]
		private Vector3 m_ToValue;
		[SerializeField, CanResetCurve]
		protected AnimationCurve m_CurveX = new AnimationCurve(new Keyframe(0, 0));
		[SerializeField, CanResetCurve]
		protected AnimationCurve m_CurveY = new AnimationCurve(new Keyframe(0, 0));
		[SerializeField, CanResetCurve]
		protected AnimationCurve m_CurveZ = new AnimationCurve(new Keyframe(0, 0));
		...
		private void Reset() {
			...
			m_Curves = new []{m_CurveX, m_CurveY, m_CurveZ};
		}

		private Vector3 GetValue(float progress) {
		...
		private void SetValue(float progress, Vector3 value) {
			for (int part = 0; part < 3; part++) {
		...
		protected virtual Vector3 TargetValue { get; set; }
		...
		public override void Capture(float progress) {
			Vector3 targetValue = TargetValue;
			...
			(m_CurveX, m_CurveY, m_CurveZ) = (m_Curves[0], m_Curves[1], m_Curves[2]);
#endif
		}
